...
static struct miu_client miu0_clients[] = {
        ...
        {"JPE_W     ",0x05,0,0,0},
        {"BACH_RW   ",0x06,0,0,0},
        {"AESDMA_RW ",0x07,0,0,0},
        {"USB20_RW  ",0x08,0,0,0},
        {"EMAC_RW   ",0x09,0,0,0},
        {"MCU51_RW  ",0x0A,0,0,0},
        {"URDMA_RW  ",0x0B,0,0,0},
        {"BDMA_RW   ",0x0C,0,0,0},
        {"MOVDMA0_RW",0x0D,0,0,0},
        {"CMDQ0_R   ",0x10,0,0,0},
        {"ISP_DMA_W ",0x11,0,0,0},
        {"ISP_DMA_R ",0x12,0,0,0},
        {"ISP_ROT_R ",0x13,0,0,0},
        {"ISP_MLOAD ",0x14,0,0,0},
        {"GOP       ",0x15,0,0,0},
        {"DIP0_R    ",0x17,0,0,0},
        {"DIP0_W    ",0x18,0,0,0},
        {"SC0_FRAME ",0x19,0,0,0},
        {"SC0_DBG_R ",0x1B,0,0,0},
        {"SC1_FRAME ",0x1C,0,0,0},
        {"SC2_FRAME ",0x1D,0,0,0},
        {"SD30_RW   ",0x1E,0,0,0},
        {"SDIO30_RW ",0x1F,0,0,0},
        {"CMDQ1_R   ",0x20,0,0,0},
        {"CMDQ2_R   ",0x21,0,0,0},
        {"ISP_DMAG  ",0x23,0,0,0},
        {"GOP1_R    ",0x24,0,0,0},
        {"GOP2_R    ",0x25,0,0,0},
        {"USB20_H_RW",0x26,0,0,0},
        {"MIIC1_RW  ",0x28,0,0,0},
        {"3DNR0_W   ",0x29,0,0,0},
        {"3DNR0_R   ",0x2A,0,0,0},
        {"CPU       ",0x70,0,0,0},
...
static ssize_t measure_all_show_hw(struct device *dev, struct device_attribute *attr, char *buf)
{
    char *str = buf;
    char *end = buf + PAGE_SIZE;
    int i=0, temp_loop_time=0;
    ...
    {
        miu0_clients[i].effi_min=0x3ff;
        miu0_clients[i].effi_max=0x0;
        miu0_clients[i].effi_avg=0;
        miu0_clients[i].bw_avg=0;
        miu0_clients[i].bw_max=0;
        miu0_clients[i].bw_min=0x3ff;
    }
    if(gmonitor_output_kmsg)
    {
        printk("Num:client\tEFFI\tBWavg\tBWmax\tBWavg/E\tBWmax/E\n");
    ...
    for(i=0; i<(sizeof(miu0_clients)/sizeof(miu0_clients[0]));i++)
    {
        if(miu0_clients[i].bw_enabled)
        {
            ...
            do{
                ...
                else if (miu0_clients[i].effi_max<temp_val)
                {
                    miu0_clients[i].effi_max=temp_val;
                }
                temp_loop_time++;
            } while (!time_after_eq(jiffies, deadline));
            ...
            do{
                ...
                else if (miu0_clients[i].bw_min>temp_val)
                {
                    miu0_clients[i].bw_min=temp_val;
                }
                temp_loop_time++;
            } while (!time_after_eq(jiffies, deadline));
            OUTREG16( (BASE_REG_MIU_PA+REG_ID_0D),0) ;//reset all
            ...
            OUTREG16( (BASE_REG_MIU_PA+REG_ID_0D), ( ((miu0_clients[i].bw_client_id << 8) & 0xFF00) | 0x60)) ;//reset
            OUTREG16( (BASE_REG_MIU_PA+REG_ID_0D), ( ((miu0_clients[i].bw_client_id << 8) & 0xFF00) | 0x61)) ;//set to read peak
            deadline = jiffies + gmonitor_duration*HZ/1000;
            do{
                msleep(gmonitor_interval); //mdelay(gmonitor_interval);
            } while (!time_after_eq(jiffies, deadline));
            miu0_clients[i].bw_max_div_effi=INREG16((BASE_REG_MIU_PA+REG_ID_0E));
            
            OUTREG16( (BASE_REG_MIU_PA+REG_ID_0D), ( ((miu0_clients[i].bw_client_id << 8) & 0xFF00) | 0x20)) ;//reset
            OUTREG16( (BASE_REG_MIU_PA+REG_ID_0D), ( ((miu0_clients[i].bw_client_id << 8) & 0xFF00) | 0x21)) ;//set to read peak
            deadline = jiffies + gmonitor_duration*HZ/1000;
            do{
                msleep(gmonitor_interval); //mdelay(gmonitor_interval);
            } while (!time_after_eq(jiffies, deadline));
            miu0_clients[i].bw_avg_div_effi=INREG16((BASE_REG_MIU_PA+REG_ID_0E));
            ...
            if(gmonitor_output_kmsg)
            {
                ...
                miu0_clients[i].bw_avg*100/1024, (miu0_clients[i].bw_avg*10000/1024)%100,
                miu0_clients[i].bw_max*100/1024, (miu0_clients[i].bw_max*10000/1024)%100,
                miu0_clients[i].bw_avg_div_effi*100/1024,
                (miu0_clients[i].bw_avg_div_effi*10000/1024)%100,
                miu0_clients[i].bw_max_div_effi*100/1024,
                (miu0_clients[i].bw_max_div_effi*10000/1024)%100);
                #if 0
                printk("effi: %d~%d, bw: %d~%d, bw_avg_div_effi: %d, bw_max_div_effi: %d\n", 
                miu0_clients[i].effi_min,
                miu0_clients[i].effi_max,
                miu0_clients[i].bw_min,
                miu0_clients[i].bw_max,
                miu0_clients[i].bw_avg_div_effi,
                miu0_clients[i].bw_max_div_effi);
                #endif
            }
            else
            {
...
DEVICE_ATTR(measure_all_hw, 0644, measure_all_show_hw, measure_all_store);
