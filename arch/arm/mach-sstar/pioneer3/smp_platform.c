...
extern int sstar_platform_cpu_kill(unsigned int cpu);
extern void sstar_platform_cpu_die(unsigned int cpu);
extern int sstar_platform_cpu_disable(unsigned int cpu);
extern void sstar_secondary_startup(void);
void sstar_smp_init_cpus(void);
void sstar_smp_prepare_cpus(unsigned int max_cpus);
...
int sstar_platform_cpu_kill(unsigned int cpu)
{
	return 1;
}
 ...
 */
void __ref sstar_platform_cpu_die(unsigned int cpu)
{
	int spurious = 0;
//	printk(KERN_ERR "[%s]\n",__FUNCTION__);
...
int sstar_platform_cpu_disable(unsigned int cpu)
{
	/*
...
//void __cpuinit sstar_secondary_init(unsigned int cpu)
void sstar_secondary_init(unsigned int cpu)
{
//	printk(KERN_ERR "[%s]\n",__FUNCTION__);
...
//int __cpuinit sstar_boot_secondary(unsigned int cpu, struct task_struct *idle)
int sstar_boot_secondary(unsigned int cpu, struct task_struct *idle)
{
...
void __init sstar_smp_prepare_cpus(unsigned int max_cpus)
{
	int i;
//	u32 scu_ctrl;
//	printk(KERN_ERR "[%s]\n",__FUNCTION__);
...
//	printk("[306]!!  scu_enable\n");
	 ...
	 */
	do{
		OUTREG16(SECOND_START_ADDR_LO,(virt_to_phys(sstar_secondary_startup) & 0xFFFF));
	}while(INREG16(SECOND_START_ADDR_LO)!=(virt_to_phys(sstar_secondary_startup) & 0xFFFF));

	do{
		OUTREG16(SECOND_START_ADDR_HI,(virt_to_phys(sstar_secondary_startup)>>16));
	}while(INREG16(SECOND_START_ADDR_HI)!=(virt_to_phys(sstar_secondary_startup)>>16));

	__cpuc_flush_kern_all();
}

void __init sstar_smp_init_cpus(void)
{
...
struct smp_operations __initdata sstar_smp_ops  = {
	.smp_init_cpus		= sstar_smp_init_cpus,
	.smp_prepare_cpus	= sstar_smp_prepare_cpus,
	.smp_secondary_init	= sstar_secondary_init,
	.smp_boot_secondary	= sstar_boot_secondary,

#ifdef CONFIG_HOTPLUG_CPU
    .cpu_kill       = sstar_platform_cpu_kill,
	.cpu_die		= sstar_platform_cpu_die,
	.cpu_disable    = sstar_platform_cpu_disable,
#endif
};
