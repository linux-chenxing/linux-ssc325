                                 ...
                                 struct msb250x_request *req)
{
    ...
    count = min((u16) ep->ep.maxpacket, (u16) (req->req.length - req->req.actual));

    ep_num = IS_ERR_OR_NULL(_ep->desc)? 0 : usb_endpoint_num(_ep->desc);

    if (0 < count)
    {
        ms_writesb((req->req.buf + req->req.actual), OTG0_EP_FIFO_ACCESS_L(ep_num), count);
    }
                                 ...
                                 struct msb250x_request *req)
{
    ...
    ep_num = IS_ERR_OR_NULL(_ep->desc)? 0 : usb_endpoint_num(_ep->desc);
    count = ms_readw(MSB250X_OTG0_EP_RXCOUNT_L_REG(ep_num));

    if (count > (req->req.length - req->req.actual))
    {
    ...
    count = min((u32) count, (u32) (req->req.length - req->req.actual));

    if (0 < count)
    {
        count = min(count, (u32) (req->req.length - req->req.actual));

        ms_readsb((req->req.buf + req->req.actual), OTG0_EP_FIFO_ACCESS_L(ep_num), count);
...
void msb250x_udc_init_OTG(struct msb250x_udc *udc)
{
    ...
    struct otg0_usb_cfg0_l* pst_usb_cfg0_l = (struct otg0_usb_cfg0_l*) &usb_cfg_l;
    ...
    pst_usb_cfg0_l->bSRST_N = 0; //low active
