...
static int phy_infinity6e_sata_init(struct phy *phy)
{
    ...
    //CLRREG16(dphy_base + 0x2a*4, 0x900);
    //SETREG16(dphy_base + 0x2a*4, 0x800); // rg_ssusb_tx_imp_sel
    // 2. RX R50
    SETREG16(dphy_base + 0x0e*4, 0x04); // rg_force_rx_imp_sel
    //CLRREG16(dphy_base + 0x3c*4, 0x110);
    //SETREG16(dphy_base + 0x3c*4, 0x100); // rg_ssusb_rx_imp_sel
    // 3. BGR INTR
    SETREG16(dphy_base + 0x46*4, 0x2000); // rg_force_iext_intr_ctrl
    //CLRREG16(dphy_base + 0x50*4, 0x5000);
    //SETREG16(dphy_base + 0x50*4, 0x8000); // rg_ssusb_iext_intr_ctrl

    SETREG16(dphy_base + 0x26*4, 0xf);
    ...
    SETREG16(dphy_base + 0x40*4, 0x3b99);
    ...
    SETREG16(0x1F000000+(0x1524<<9)+(0x11<<2), 0x0020); //CDR JTOL Improvement ( Improve BBPD of CDR if 16’h11 bit[5] = 1’b1. Default with MTK phyA mode.
    SETREG16(0x1F000000+(0x1523<<9)+(0x29<<2), 0x0400|0x0200|0x0100|0x0080|0x0040); //LPFS filter threshold
