...
#define MIR3DA_NSA_ODR_SECTION                          { NSA_REG_ODR_AXIS_DISABLE,     0x0F,   0x05    },                                  \
...
#define MIR3DA_NSA_ODR_DURATION                         { NSA_REG_ACTIVE_DURATION,     0xFF,   0x33    },
...
RETRY_READ_XYZ:
    ...
    // work around for continuous 0xff
    #if 1
    if(tmp_data[5] == 0XFF){
        MI_ERR("mir3da_raw:[%x %x %x %x %x %x],retry...",tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3], tmp_data[4], tmp_data[5]);
        if(u8Retry == 0)
            return -1;
        u8Retry --;
        goto RETRY_READ_XYZ;
    ...
    #else
    for (i = 0; i < 6; i++) {
        if (i < 5 && tmp_data[i] == 0xFF) {
            if (tmp_data[i+1] == 0xFF) {
                MI_ERR("mir3da_raw:[%x %x %x %x %x %x]",tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3], tmp_data[4], tmp_data[5]);
                if(u8Retry == 0)
                    return -1;
                u8Retry --;
                goto RETRY_READ_XYZ;
    ...
    *z = ((short)(tmp_data[5] << mir3da_gsensor_drv.obj[gsensor_mod].data.data_fmt.msbw | tmp_data[4])) >> (8-mir3da_gsensor_drv.obj[gsensor_mod].data.data_fmt.lsbw);
    MI_DATA("mir3da_raw: x=%d, y=%d, z=%d [%x %x %x %x %x %x]",  *x, *y, *z, tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3], tmp_data[4], tmp_data[5]);
...
int mir3da_set_odr(MIR_HANDLE handle, int delay)
{
    ...
    //active_dur register
    res = mir3da_register_mask_write(handle, mir3da_gsensor_drv.obj[gsensor_mod].odr_duration[0].addr,
            mir3da_gsensor_drv.obj[gsensor_mod].odr_duration[0].mask,mir3da_gsensor_drv.obj[gsensor_mod].odr_duration[0].value);
    if (res != 0) {
        MI_ERR("%s:ODR DUR %02x=%02x\n",__func__,mir3da_gsensor_drv.obj[gsensor_mod].odr_duration[0].addr, mir3da_gsensor_drv.obj[gsensor_mod].odr_duration[0].value);
        return res;
    }
...
int mir3da_open_interrupt(MIR_HANDLE handle, int num, int on, unsigned int th)
{
    int   res = 0;
    ...
    res |= mir3da_register_mask_write(handle,NSA_REG_ACTIVE_THRESHOLD,0xff,th);
    if (on) {
        res |= mir3da_register_mask_write(handle,NSA_REG_INTERRUPT_SETTINGS1,0xff,0x07);
        switch(num) {
            case 0:
