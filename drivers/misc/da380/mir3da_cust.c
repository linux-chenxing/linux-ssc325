...
#define MIR3DA_DRV_NAME                     "da380_i2c"
                    ...
                    const char *buf, size_t count)
{
    ...
    mir3da_open_interrupt(handle, num, on, slope_th);
    return count;
}
...
static DEVICE_ATTR(axis_data,             0440,  mir3da_axis_data_show,          NULL);
...
static DEVICE_ATTR(primary_offset,        0440,  mir3da_primary_offset_show,     NULL);
static DEVICE_ATTR(version,               0440,  mir3da_version_show,            NULL);
static DEVICE_ATTR(vendor,                0440,  mir3da_vendor_show,             NULL);
...
static int mir3da_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    result = mir3da_set_enable(mir_handle, 1);
    if (result) {
        MI_ERR("%s:set enable fail!!\n",__func__);
        return result;
    }

    result = mir3da_set_odr(mir_handle, 11);
    if (result) {
        MI_ERR("%s:set enable fail!!\n",__func__);
        return result;
    }
