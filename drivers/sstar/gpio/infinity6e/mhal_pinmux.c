...
static S32 HalPadSetMode_MISC(U32 u32PadID, U32 u32Mode)
{
    ...
    case PAD_FUART_RTS:
        ...
        else if(u32Mode == PINMUX_FOR_SPI0_MODE_2)
        {
            _HalPadDisablePadMux(PINMUX_FOR_EJ_MODE_1);
            _GPIO_W_WORD_MASK(_RIUA_16BIT(PADTOP_BANK,REG_SPI0_MODE), BIT1, REG_SPI0_MODE_MASK);
        }
        else if(u32Mode == PINMUX_FOR_SPI1_MODE_1)
        {
            _HalPadDisablePadMux(PINMUX_FOR_EJ_MODE_1);
            _GPIO_W_WORD_MASK(_RIUA_16BIT(PADTOP_BANK,REG_SPI1_MODE), BIT4, REG_SPI1_MODE_MASK);
        }
        else
        {
            return -1;
