...
#define SPI_MIU0_BUS_BASE                  ARM_MIU0_BUS_BASE
#define SPI_MIU1_BUS_BASE                  ARM_MIU1_BUS_BASE
...
static int mstar_spi_setup(struct spi_device *spi)
{
    ...
    if(bs->bits_per_word != spi->bits_per_word)
    {
        bs->bits_per_word = spi->bits_per_word;
        SUPPORT_DMA = (bs->bits_per_word % 8 == 0) ? bs->use_dma : 0;
        ...
        HAL_MSPI_SET_FRAMECFG(bs, bs->u8channel, spi->bits_per_word);
...
static int mstar_spi_probe(struct platform_device *pdev)
{
	...
	master->bits_per_word_mask = SPI_BPW_RANGE_MASK(1, MSPI_MAX_SUPPORT_BITS);
	...
	master_spi1->bits_per_word_mask = SPI_BPW_RANGE_MASK(1, MSPI_MAX_SUPPORT_BITS);
