...
#define MSPI0_MODE          0x68 //bit0~bit1
    #define MSPI0_MODE_MASK 0x07
#define MSPI1_MODE          0x68 //bit4~bit5
    #define MSPI1_MODE_MASK 0x70
#define EJTAG_MODE          0x60
    #define EJTAG_MODE_1    0x0080
    #define EJTAG_MODE_MASK 0x0180
//-------------------------------------------------------------------------------------------------
//  Local Defines
//-------------------------------------------------------------------------------------------------
...
static u8 HAL_MSPI_FRAMEConfig(struct mstar_spi *bs, U8 u8Channel, MSPI_FrameConfig  *ptFrameConfig)
{
    u8 errnum = E_MSPI_OK;
    u8 u8Index = 0;
    ...
    if(ptFrameConfig == NULL)
    {
        HAL_MSPI_Reset_FrameConfig(bs, u8Channel);
        return E_MSPI_OK;
    }
    // read buffer bit config
    for(u8Index = 0; u8Index < MAX_READ_BUF_SIZE; u8Index++)
    {
        ...
        {
            HAL_MSPI_SetPerFrameSize(bs, u8Channel, MSPI_READ_INDEX,  u8Index, ptFrameConfig->u8RBitConfig[u8Index]);
        }
    }
    //write buffer bit config
    for(u8Index = 0; u8Index < MAX_WRITE_BUF_SIZE; u8Index++)
    {
        ...
        {
            HAL_MSPI_SetPerFrameSize(bs, u8Channel, MSPI_WRITE_INDEX,  u8Index, ptFrameConfig->u8WBitConfig[u8Index]);
        }
    }
    return errnum;
}

u8 HAL_MSPI_SET_FRAMECFG(struct mstar_spi *bs, U8 u8Channel, int bits_per_word)
{
    ...
    HAL_MSPI_FRAMEConfig(bs, u8Channel, &stFrameConfig);
		...
		struct spi_transfer *tfr)
{
	...
	MDrv_MSPI_ChipSelect(bs,spi->master->bus_num,1,0);
	...
	if(bs->tx_buf != NULL){
	    ...
	    MDrv_MSPI_Write(bs, spi->master->bus_num,(U8 *)bs->tx_buf,(U16)bs->len);
    }

	if(bs->rx_buf != NULL){

		MDrv_MSPI_Read(bs,spi->master->bus_num,(U8 *)bs->rx_buf,(U16)bs->len);
		...
		struct spi_transfer *tfr, bool cs_change)
{
	...
	if (cs_change){
		/* Clear TA flag */
		MDrv_MSPI_ChipSelect(bs,spi->master->bus_num,0,0);
...
static int mstar_spi_probe(struct platform_device *pdev)
{
	...
	mstar_master0->bits_per_word_mask = SPI_BPW_RANGE_MASK(1, MSPI_MAX_SUPPORT_BITS);
	...
	mstar_master1->bits_per_word_mask = SPI_BPW_RANGE_MASK(1, MSPI_MAX_SUPPORT_BITS);
