...
unsigned int check_rsqslot(reservoir_t *rvr, unsigned int expect)
{
    ...
    if (!rvr)
        return 0;
    Chip_Inv_Cache_Range((unsigned long)rvr, sizeof(reservoir_t));
    prsq  = &(rvr->rsq);
...
unsigned int gc_rsqslot(reservoir_t *rvr, unsigned int expect)
{
    ...
    Chip_Flush_Cache_Range((unsigned long)rvr, sizeof(reservoir_t));
...
void *begin_rsqslot(reservoir_t *rvr, unsigned int expect)
{
    ...
    slot = (slot_t*)(prsq->sndr_wp);
    Chip_Inv_Cache_Range((unsigned long)slot, sizeof(slot_t));
    slot->rese = rvr;
    Chip_Flush_Cache_Range((unsigned long)slot, sizeof(slot_t));
    /* |-------Wssssssss-R--O----------|  */
    /* sbuf   ptr        ^ oob        end */
