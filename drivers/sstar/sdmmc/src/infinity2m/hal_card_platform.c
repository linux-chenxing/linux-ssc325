...
#else
            HalPadSetMode(PINMUX_FOR_SDIO_MODE_1);
#endif
        }
    }
    else if(eIP == EV_IP_FCIE2)
...
//            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PM_SLEEP_BANK, 0x28), BIT14_T);   //SD_CDZ mode en
            ...
            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PM_GPIO_BANK, 0x29), BIT00_T);    //PAD_SD1_IO6 input mode
...
//            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_GPI_INT_BANK, 0x15), BIT00_T);    //reg_fiq_raw_sel = 1
            CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_GPI_INT_BANK, 0x02), BIT10_T);    //pin 42 fiq unmask
            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_GPI_INT_BANK, 0x0C), BIT10_T);    //pin 42 clear
            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_GPI_INT_BANK, 0x12), BIT10_T);    //reg_fiq_pol for pin 42
        }
    }
}
...
BOOL_T Hal_CARD_GetGPIOState(GPIOEmType eGPIO, PADEmType ePAD)
{
    ...
    if( eGPIO==EV_GPIO2 ) //EV_GPIO2 for Slot 1
    {
        ...
        {
//            U16_T u16RegRaw = 0, po;

//            u16Reg = CARD_REG(GET_CARD_REG_ADDR(A_GPI_INT_BANK, 0x32)) & BIT10_T;//FIQ final status
            u16Reg = CARD_REG(GET_CARD_REG_ADDR(A_GPI_INT_BANK, 0x37)) & BIT10_T;//FIQ raw status
//            u16RegRaw = CARD_REG(GET_CARD_REG_ADDR(A_GPI_INT_BANK, 0x37)) & BIT10_T;//FIQ raw status

//            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_GPI_INT_BANK, 0x0C), BIT10_T);//clear pin 42 final status

//            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_GPI_INT_BANK, 0x0C), 0);//do not clear pin 42 status
//            u16Reg = CARD_REG(GET_CARD_REG_ADDR(A_GPI_INT_BANK, 0x37) & BIT10_T);//FIQ raw status
            //u16Reg = CARD_REG(GET_CARD_REG_ADDR(A_PM_GPIO_BANK, 0x47)) & BIT02_T;
//            po = CARD_REG(GET_CARD_REG_ADDR(A_GPI_INT_BANK, 0x12));    //reg_fiq_pol for pin 42
//            printk(SD_RED"--- gpi:fin%X raw:%x po:%X"SD_DEF"\n", u16Reg, u16RegRaw, po);
        }
...
void Hal_CARD_SetGPIOIntAttr(GPIOEmType eGPIO, PADEmType ePAD, GPIOOptEmType eGPIOOPT, unsigned int irq)
{
    ...
    else if((eGPIOOPT==EV_GPIO_OPT3))  //sd polarity _HI Trig for remove
    {
        ...
        if(ePAD == EV_PAD2)
        {
            printk(SD_RED"opt 3"SD_DEF"\n");
            CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_GPI_INT_BANK, 0x12), BIT10_T);    //reg_fiq_pol for pin 42
            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_GPI_INT_BANK, 0x0C), BIT10_T);//clear pin 42 final status
        }
