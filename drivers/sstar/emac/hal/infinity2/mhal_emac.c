...
static void _MHal_EMAC_Clk(void* hal, int bOn)
{
    ...
    //0x1033_32[4:0] = 0x8 reg_ckg_emac_ahb
    //0x100b_61[2:0] = 0x0 reg_ckg_emac_tx
    //0x100b_60[10:8] = 0x0 reg_ckg_emac_rx
    if (bOn)
    {
        uRegVal = MHal_EMAC_ReadReg8(EMAC_RIU_REG_BASE, REG_BANK_CLKGEN1, 0x64);
        uRegVal = (uRegVal&~0x1F)|0x8;
        MHal_EMAC_WritReg8(EMAC_RIU_REG_BASE,REG_BANK_CLKGEN1, 0x64, uRegVal);

        uRegVal = MHal_EMAC_ReadReg8(EMAC_RIU_REG_BASE, REG_BANK_CLKGEN0, 0xC2);
        uRegVal = (uRegVal&~0x7);
        MHal_EMAC_WritReg8(EMAC_RIU_REG_BASE,REG_BANK_CLKGEN0, 0xC2, uRegVal);

        uRegVal = MHal_EMAC_ReadReg8(EMAC_RIU_REG_BASE,REG_BANK_CLKGEN0, 0xC1);
        uRegVal = (uRegVal&~0x7);
        MHal_EMAC_WritReg8(EMAC_RIU_REG_BASE, REG_BANK_CLKGEN0, 0xC1, uRegVal);
    }
    else
    {
        MHal_EMAC_WritReg8(EMAC_RIU_REG_BASE, REG_BANK_CLKGEN1, 0x64, 0x01);
        MHal_EMAC_WritReg8(EMAC_RIU_REG_BASE, REG_BANK_CLKGEN0, 0xC1, 0x01);
        MHal_EMAC_WritReg8(EMAC_RIU_REG_BASE, REG_BANK_CLKGEN0, 0xC3, 0x01);
        MHal_EMAC_WritReg8(EMAC_RIU_REG_BASE, REG_BANK_CLKGEN0, 0xC2, 0x01);
        MHal_EMAC_WritReg8(EMAC_RIU_REG_BASE, REG_BANK_CLKGEN0, 0xC4, 0x01);
    }
}
...
static void _MHal_EMAC_albany_clk_on(void* hal)
{
    ...
    MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY0, 0xaa, 0x19);   // tin17_s2
    MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY0, 0xac, 0x19);   // tin18_s2
    MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY0, 0xad, 0x19);
    MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY0, 0xae, 0x19);   // tin19_s2
    MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY0, 0xaf, 0x19);

    MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY2, 0xe8, 0x00);
    MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY0, 0xaa, 0x19);
	...
	// 10t_8bt
    MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY0, 0xb4, 0x5a); // def 32, counterwidth_max
    MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY0, 0xb6, 0x50); // def 28, counter_width_h (normal)
    MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY0, 0xff, 0x1a); // def 10, counter_width_h (lp)

    // snr check threshold define when snr locked
    MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY2, 0x93, 0x04);
	//Fix EEE TX issue
    MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY2, 0xec, 0x10);
	//release snr setting after 1ms
    MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY2, 0x94, 0x18);
	//TR K1 for 28nm process
    MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY1, 0x33, 0x6d);
    //set CLKsource to hv
    //@@
