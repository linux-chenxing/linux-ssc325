...
struct miu_client miu0_clients[] =
{
    ...
    {"RSVD      ",0x13,0,0,0,0},
    {"RSVD      ",0x14,0,0,0,0},
    {"ISP_DMAGR ",0x15,0,0,0,0},
    {"SC_ROT    ",0x16,0,0,0,0},
    {"RDMA_SC0  ",0x17,0,0,0,0},
    {"WDMA_SC4  ",0x18,0,0,0,0},
    {"WDMA_SC5  ",0x19,0,0,0,0},
    {"GE        ",0x1A,0,0,0,0},
    {"ISP_3DNR0W",0x1B,0,0,0,0},
    {"ISP_3DNR0R",0x1C,0,0,0,0},
    {"ISP_STA   ",0x1D,0,0,0,0},
    {"ISP_DMA   ",0x1E,0,0,0,0},
    {"RSVD      ",0x1F,0,0,0,0},
    {"USB20_1   ",0x20,0,0,0,0},
    {"USE20_0   ",0x21,0,0,0,0},
    {"ISP_DMAGW ",0x22,0,0,0,0},
    {"GOP_DISP1 ",0x23,0,0,0,0},
    {"CMDQ_R    ",0x24,0,0,0,0},
    {"GOP0      ",0x25,0,0,0,0},
    {"RSVD      ",0x26,0,0,0,0},
    {"RSVD      ",0x27,0,0,0,0},
...
MS_BOOL HAL_MIU_GetHitProtectInfo(MS_U8 u8MiuSel, MIU_PortectInfo *pInfo)
{
    ...
    if (REG_MMU_PROTECT_HIT_FALG & u16MmuRet) /*indicate protection area been accessed*/
    {
		...
		if((char *)halClientIDTName((MS_U8)(GET_MMU_HIT_CLIENT(u16MmuRet))) != NULL)
    ...
    if (REG_MIU_PROTECT_HIT_FALG & u16Ret)
    {
		...
		if((char *)halClientIDTName((MS_U8)(GET_HIT_CLIENT(u16Ret))) != NULL)
...
int HAL_MMU_SetRegion(unsigned short u16Region, unsigned short u16ReplaceRegion)
{
    ...
    {
        case E_MMU_PGSZ_128:
            u8RegShiftVal = 6;  //128KB:[31:26],region control 5bit
            break;
        case E_MMU_PGSZ_256:
            u8RegShiftVal = 5;  //256KB:[31:27],region control 4bit
            break;
        case E_MMU_PGSZ_512:
            u8RegShiftVal = 4;  //512KB:[31:28],region control 3bit
            break;
        default:
            break;
    }
