...
static int MDev_GMAC_BGsend(struct net_device* dev, phys_addr_t addr, int len )
{
    ...
    {
        Chip_Flush_Cache_Range((unsigned int)skb_addr&0x0FFFFFFFUL, len);
    }
    else
    {
        Chip_Flush_Cache_Range(0, 0xFFFFFFFFUL);
    }
    #elif defined(CONFIG_ARM)
        Chip_Flush_Cache_Range(0, 0xFFFFFFFFUL);
    #else
...
static int MDev_GMAC_tx (struct sk_buff *skb, struct net_device *dev)
{
    ...
    #ifdef CHIP_FLUSH_READ
    #if defined(CONFIG_MIPS)
        Chip_Flush_Cache_Range((unsigned int)skb&0x0FFFFFFFUL, skb->len);
    #endif
    #endif
    ...
    #ifdef CHIP_FLUSH_READ
        #if defined(CONFIG_MIPS)
            Chip_Flush_Cache_Range((u32)(&(LocPtr->tx_desc_list[LocPtr->tx_desc_write_index])) & 0x0FFFFFFF, sizeof(LocPtr->tx_desc_list[LocPtr->tx_desc_write_index]));
        #endif
    #endif
        LocPtr->tx_desc_write_index = 0;
    }
...
static int MDev_GMAC_tx (struct sk_buff *skb, struct net_device *dev)
{
    ...
    {
        Chip_Flush_Cache_Range((unsigned int)skb_addr&0x0FFFFFFFUL, skb->len);
    }
    else
    {
        Chip_Flush_Cache_Range(0, 0xFFFFFFFFUL);
        ...
        TO_DO;Chip_Flush_Cache_Range(0, 0xFFFFFFFFUL);
    #else
        #ERROR
    #endif
#endif
...
static int MDev_GMAC_tx (struct sk_buff *skb, struct net_device *dev)
{
    ...
    #elif defined(CONFIG_ARM) || defined(CONFIG_ARM64)
        TO_DO;Chip_Flush_Cache_Range((unsigned int)skb_addr&0x0FFFFFFFUL, skb->len);
...
static int MDev_GMAC_rx (struct net_device *dev)
{
    ...
    do
    {
#ifdef GMAC_CHIP_FLUSH_READ
    #if defined(CONFIG_MIPS)
        Chip_Inv_Cache_Range((u32)(&(LocPtr->rx_desc_list[LocPtr->rx_desc_read_index])) & 0x0FFFFFFFUL, sizeof(struct rx_descriptor));
    #elif defined(CONFIG_ARM) || defined(CONFIG_ARM64)
        ...
        if (skb != NULL)
        {
           ...
           {
               Chip_Inv_Cache_Range((u32)(p_recv) & 0x0FFFFFFFUL, pktlen);
           }
           else
           {
               Chip_Inv_Cache_Range(0, 0xFFFFFFFFUL);
           }
        #elif defined(CONFIG_ARM) || defined(CONFIG_ARM64)
...
#ifdef GMAC_CHIP_FLUSH_READ
    #if defined(CONFIG_MIPS)
        Chip_Flush_Cache_Range((u32)(&(LocPtr->rx_desc_list[LocPtr->rx_desc_read_index].addr)) & 0x0FFFFFFFUL, sizeof(LocPtr->rx_desc_list[LocPtr->rx_desc_read_index].addr));
    #elif defined(CONFIG_ARM) || defined(CONFIG_ARM64)
